        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {data.skills.map((category, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`p-8 transition-all duration-700 relative
                ${era === '1800s' ? 'border-l-4 border-accent bg-accent/5 font-serif italic' : 
                  ...
                  'bg-black/50 border border-accent/20 rounded-none shadow-[0_0_15px_rgba(6,182,212,0.05)] hover:border-accent/40'}
              `}
            >
                  ...
                  <motion.div
                    ...
                    className={`px-4 py-2 text-xs font-retro tracking-wider transition-all
                      ${era === '1800s' ? 'border-b border-accent/30 text-primary' : 
                        ...
                        'text-accent border border-accent/25 hover:border-accent hover:bg-accent/5 rounded-none pl-3 pr-3 py-1 font-bold font-heading shadow-[0_0_5px_rgba(6,182,212,0.05)]'}
                    `}
                  >
                    {skill}
                  </motion.div>
